...
public interface ResultMappingFunction<T, R> extends Function<Result<T>, Result<R>> {
	...
	/**
	 * Creates a new result mapping function that returns the input result as output result.<br>
	 * The output result is the same as the input result.<br>
	 *
	...
	static <T, R> @NotNull ResultMappingFunction<T, R> throwable(@NotNull ThrowableFunction<T, R, ? extends Throwable> function) {
		...
		return result -> {
			...
			try {
				return Result.success(function.apply(result.resultOrThrow()));
