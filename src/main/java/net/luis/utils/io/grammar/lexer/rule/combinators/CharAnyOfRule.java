...
package net.luis.utils.io.grammar.lexer.rule.combinators;

import net.luis.utils.io.grammar.lexer.CharRuleMatch;
import net.luis.utils.io.grammar.lexer.rule.CharRule;
import net.luis.utils.io.grammar.lexer.stream.CharStream;
...
/**
 * A character rule that matches any of the provided character rules (ordered alternation).<br>
 * The first matching rule will be used to create the {@link CharRuleMatch}.<br>
 * This rule is useful for creating complex matching logic by combining multiple rules.<br>
 *
 * @author Luis-St
 *
 * @param charRules The list of character rules to match against
 */
public record CharAnyOfRule(
	@NonNull List<CharRule> charRules
) implements CharRule {
	
	/**
	 * Constructs a new any of character rule with the given character rules.<br>
	 *
	 * @param charRules The list of character rules to match against
	...
	public @NonNull CharRule not() {
		return new CharSequenceRule(this.charRules.stream().map(CharRule::not).toList()); // Negation using De Morgan's laws
	}
}
