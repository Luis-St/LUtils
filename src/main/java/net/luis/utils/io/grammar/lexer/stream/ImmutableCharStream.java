...
/**
 * An immutable implementation of the {@link CharStream} interface.<br>
 * This class represents a stream of characters whose current index cannot be modified after creation.<br>
 * The following operations are not supported and will throw an {@link UnsupportedOperationException} if called:<br>
 * <ul>
 *     <li>{@link #advanceBy(int)}</li>
 ...
 */
public class ImmutableCharStream implements CharStream {
	 ...
	 */
	ImmutableCharStream(@NonNull String input, int currentIndex) {
	...
	public char readChar() {
		throw new UnsupportedOperationException("Immutable character stream cannot be modified");
	}
	...
	public @NonNull CharStream copyWithIndex(int index) {
		return new ImmutableCharStream(this.input, Mth.clamp(index, 0, this.input.length()));
	}
}
