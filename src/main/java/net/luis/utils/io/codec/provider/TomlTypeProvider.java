...
public final class TomlTypeProvider implements TypeProvider<TomlElement> {
	...
	@Override
	public <X extends Exception> @NonNull TomlElement get(@Nullable TomlElement type, @Nullable String key, @NotNull Function<String, X> exceptionConstructor) throws X {
		Objects.requireNonNull(exceptionConstructor, "Exception constructor must not be null");
		...
		if (element == null) {
			throw exceptionConstructor.apply("Key '" + key + "' does not exist in toml table '" + type + "'");
		}
		return element;
	}
