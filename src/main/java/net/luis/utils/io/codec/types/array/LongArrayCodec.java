 ...
 * @author Luis-St
 */
public class LongArrayCodec extends AbstractCodec<long[], PrimitiveArrayConstraintConfig<long[]>> implements PrimitiveArrayConstraint<long[], LongArrayCodec> {
	...
	@Override
	public @NonNull LongArrayCodec apply(@NonNull UnaryOperator<PrimitiveArrayConstraintConfig<long[]>> configModifier) {
		Objects.requireNonNull(configModifier, "Config modifier must not be null");
		
		return new LongArrayCodec(
			configModifier.apply(this.getConstraintConfig().orElse(PrimitiveArrayConstraintConfig.longArray()))
		);
	}
	
	@Override
	public <R> @NonNull Result<R> encodeStart(@NonNull TypeProvider<R> provider, @NonNull R current, long @Nullable [] value) {
		Objects.requireNonNull(provider, "Type provider must not be null");
		Objects.requireNonNull(current, "Current value must not be null");
		if (value == null) {
			return Result.error("Unable to encode null as long array using '" + this + "'");
		}
	...
	public <R> @NonNull Result<long[]> decodeStart(@NonNull TypeProvider<R> provider, @NonNull R current, @Nullable R value) {
		...
		Objects.requireNonNull(current, "Current value must not be null");
		if (value == null) {
			return Result.error("Unable to decode null value as long array using '" + this + "'");
		}
		
		Result<List<Long>> result = this.internalCodec.decodeStart(provider, current, value);
		if (result.isError()) {
			return Result.error(result.errorOrThrow());
		}
		
		List<Long> list = result.resultOrThrow();
	...
	public String toString() {
		return this.getConstraintConfig().map(config -> {
			return "ConstrainedLongArrayCodec[constraints=" + config + "]";
		}).orElse("LongArrayCodec");
	}
}
