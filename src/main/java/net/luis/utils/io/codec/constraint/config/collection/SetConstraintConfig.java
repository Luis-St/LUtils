...
/**
 * Configuration record for set constraints on set types.<br>
 * <p>
 *     This record stores the constraint values for {@link SetConstraint}.<br>
 *     It provides constraints for set equality, collection membership, and size validation.
 * </p>
 * <p>
 *     The equalTo field uses {@link Pair} where the first value is the set and
 *     the second value indicates negation (false=equalTo, true=notEqualTo).
 * </p>
 * <p>
 *     The in field uses {@link Pair} where the first value is the set of sets and
 ...
 * @param equalTo The set equality constraint as a pair of (value, negated) where negated=false means equalTo and negated=true means notEqualTo
 * @param in The set collection constraint as a pair of (values, negated) where negated=false means in and negated=true means notIn
 * @param size The size constraint configuration
 * @param custom A custom constraint implementation
 */
public record SetConstraintConfig<T>(
	@NonNull Optional<Pair<Set<T>, Boolean>> equalTo,
	@NonNull Optional<Pair<Set<Set<T>>, Boolean>> in,
	@NonNull Optional<SizeConstraintConfig> size,
	@NonNull Optional<Constraint<Set<T>>> custom
) implements ConstraintConfig<Set<T>> {
	
	/**
	 * Constructs a new set constraint config with the specified parameters.<br>
	 *
	 * @param equalTo The set equality constraint as a pair of (value, negated) where negated=false means equalTo and negated=true means notEqualTo
	 * @param in The set collection constraint as a pair of (values, negated) where negated=false means in and negated=true means notIn
	 * @param size The size constraint configuration
	...
	/**
	 * Creates an unconstrained set configuration with no constraints applied.<br>
	 *
	 * @param <T> The element type of the set
	 * @return An unconstrained set constraint config
	 */
	...
	public @NonNull SetConstraintConfig<T> withEqualTo(@NonNull Set<T> value) {
		...
		return new SetConstraintConfig<>(Optional.of(Pair.of(Set.copyOf(value), false)), this.in, this.size, this.custom);
	}
	...
	public @NonNull SetConstraintConfig<T> withNotEqualTo(@NonNull Set<T> value) {
		...
		return new SetConstraintConfig<>(Optional.of(Pair.of(Set.copyOf(value), true)), this.in, this.size, this.custom);
	}
	...
	public @NonNull SetConstraintConfig<T> withIn(@NonNull Collection<Set<T>> values) {
		...
		Set<Set<T>> copies = new HashSet<>();
		for (Set<T> value : values) {
			copies.add(Set.copyOf(value));
		}
		return new SetConstraintConfig<>(this.equalTo, Optional.of(Pair.of(copies, false)), this.size, this.custom);
	}
	...
	public @NonNull SetConstraintConfig<T> withNotIn(@NonNull Collection<Set<T>> values) {
		...
		return new SetConstraintConfig<>(this.equalTo, Optional.of(Pair.of(copies, true)), this.size, this.custom);
	}
	
	/**
	 * Creates a new set constraint config with the specified minimum size (inclusive).<br>
	 *
	 * @param minSize The minimum size (inclusive)
	 * @return A new config with the minimum size constraint applied
	 */
	public @NonNull SetConstraintConfig<T> withMinSize(int minSize) {
		SizeConstraintConfig newSize = this.size.orElse(SizeConstraintConfig.UNCONSTRAINED).withMinSize(minSize);
		return new SetConstraintConfig<>(this.equalTo, this.in, Optional.of(newSize), this.custom);
	}
	
	/**
	 * Creates a new set constraint config with the specified maximum size (inclusive).<br>
	 *
	 * @param maxSize The maximum size (inclusive)
	 * @return A new config with the maximum size constraint applied
	 */
	public @NonNull SetConstraintConfig<T> withMaxSize(int maxSize) {
		SizeConstraintConfig newSize = this.size.orElse(SizeConstraintConfig.UNCONSTRAINED).withMaxSize(maxSize);
		return new SetConstraintConfig<>(this.equalTo, this.in, Optional.of(newSize), this.custom);
	}
	
	/**
	 * Creates a new set constraint config with the specified exact size.<br>
	 *
	 * @param exactSize The exact size required
	 * @return A new config with the exact size constraint applied
	 */
	public @NonNull SetConstraintConfig<T> withExactSize(int exactSize) {
		SizeConstraintConfig newSize = this.size.orElse(SizeConstraintConfig.UNCONSTRAINED).withExactSize(exactSize);
		return new SetConstraintConfig<>(this.equalTo, this.in, Optional.of(newSize), this.custom);
	}
	
	/**
	 * Creates a new set constraint config with the specified size range (inclusive).<br>
	 *
	 * @param minSize The minimum size (inclusive)
	 * @param maxSize The maximum size (inclusive)
	 * @return A new config with the size range constraint applied
	 */
	public @NonNull SetConstraintConfig<T> withSizeBetween(int minSize, int maxSize) {
		SizeConstraintConfig newSize = this.size.orElse(SizeConstraintConfig.UNCONSTRAINED).withSizeBetween(minSize, maxSize);
		return new SetConstraintConfig<>(this.equalTo, this.in, Optional.of(newSize), this.custom);
	}
	
	/**
	 * Creates a new set constraint config with the specified size constraints.<br>
	 * <p>
	 ...
	 */
	public @NonNull SetConstraintConfig<T> withSize(@NonNull SizeConstraintConfig sizeConfig) {
		Objects.requireNonNull(sizeConfig, "Size config must not be null");
		return new SetConstraintConfig<>(this.equalTo, this.in, Optional.of(sizeConfig), this.custom);
	}
	...
	public @NonNull SetConstraintConfig<T> withCustom(@NonNull Constraint<Set<T>> constraint) {
		...
		return new SetConstraintConfig<>(this.equalTo, this.in, this.size, Optional.of(constraint));
	}
	//endregion
