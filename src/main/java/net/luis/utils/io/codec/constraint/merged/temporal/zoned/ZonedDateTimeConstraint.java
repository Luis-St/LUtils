...
/**
 * Constraint interface for {@link ZonedDateTime} that provides temporal validation operations.<br>
 * <p>
 ...
 *     zoned date time validation capabilities.<br>
 ...
 *     millisecond, nanosecond) for validating zoned date time values.
 * </p>
 *
 * @author Luis-St
 *
 ...
 */
@FunctionalInterface
public interface ZonedDateTimeConstraint<C> extends ApplicableConstraint<ZonedDateTimeConstraintConfig, C>, TemporalComparableConstraint<ZonedDateTime, C>, TemporalSpanConstraint<ZonedDateTime, C>, DateFieldConstraint<ZonedDateTime, C>, TimeFieldConstraint<ZonedDateTime, C> {
	
	@Override
	@NonNull C apply(@NonNull UnaryOperator<ZonedDateTimeConstraintConfig> configModifier);
	
	@Override
	default @NonNull C equalTo(@NonNull ZonedDateTime value) {
		return this.apply(config -> config.withEqualTo(value));
	}
	
	@Override
	default @NonNull C notEqualTo(@NonNull ZonedDateTime value) {
		return this.apply(config -> config.withNotEqualTo(value));
	}
	
	@Override
	default @NonNull C in(@NonNull Collection<ZonedDateTime> values) {
		return this.apply(config -> config.withIn(values));
	}
	
	@Override
	default @NonNull C notIn(@NonNull Collection<ZonedDateTime> values) {
		return this.apply(config -> config.withNotIn(values));
	}
	
	@Override
	default @NonNull C custom(@NonNull Constraint<ZonedDateTime> constraint) {
		return this.apply(config -> config.withCustom(constraint));
	}
	
	@Override
	default @NonNull C after(@NonNull ZonedDateTime value) {
		return this.apply(config -> config.withAfter(value));
	}
	
	@Override
	default @NonNull C afterOrEqual(@NonNull ZonedDateTime value) {
		return this.apply(config -> config.withAfterOrEqual(value));
	}
	
	@Override
	default @NonNull C before(@NonNull ZonedDateTime value) {
		return this.apply(config -> config.withBefore(value));
	}
	
	@Override
	default @NonNull C beforeOrEqual(@NonNull ZonedDateTime value) {
		return this.apply(config -> config.withBeforeOrEqual(value));
	}
	
	@Override
	default @NonNull C between(@NonNull ZonedDateTime min, @NonNull ZonedDateTime max) {
		return this.apply(config -> config.withBetween(min, max));
	}
	
	@Override
	default @NonNull C betweenOrEqual(@NonNull ZonedDateTime min, @NonNull ZonedDateTime max) {
