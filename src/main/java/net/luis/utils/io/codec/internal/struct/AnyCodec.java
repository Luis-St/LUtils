 ...
 */
@ApiStatus.Internal
public class AnyCodec<C> implements Codec<C> {
	 ...
	 */
	@SafeVarargs
	public AnyCodec(@NotNull Codec<? extends C>... codecs) {
	...
	public AnyCodec(@NotNull List<? extends Codec<? extends C>> codecs) {
		...
		// Store codecs with an unchecked cast - this is safe because we only use them
		// to try encoding/decoding and handle failures gracefully
		this.codecs = (List<Codec<C>>) (List<?>) List.copyOf(codecs);
	}
