 ...
 */
@ApiStatus.Internal
public class CharacterArrayCodec implements Codec<char[]> {
	...
	@Override
	public @NotNull <R> Result<char[]> decodeStart(@NotNull TypeProvider<R> provider, @Nullable R value) {
		Objects.requireNonNull(provider, "Type provider must not be null");
		if (value == null) {
			return Result.error("Unable to decode null value as char array using '" + this + "'");
		}
		
		Result<List<Character>> result = this.internalCodec.decodeStart(provider, value);
		if (result.isError()) {
