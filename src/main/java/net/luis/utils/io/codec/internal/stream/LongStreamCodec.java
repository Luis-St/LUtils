 ...
 */
@ApiStatus.Internal
public class LongStreamCodec implements Codec<LongStream> {
	...
	@Override
	public @NotNull <R> Result<LongStream> decodeStart(@NotNull TypeProvider<R> provider, @Nullable R value) {
		Objects.requireNonNull(provider, "Type provider must not be null");
		if (value == null) {
			return Result.error("Unable to decode null value as long stream using '" + this + "'");
		}
		
		Result<Stream<Long>> result = this.internalCodec.decodeStart(provider, value);
		if (result.isError()) {
