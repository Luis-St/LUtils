...
/**
 * Constraint interface for {@link LocalDateTime} that provides temporal validation operations.<br>
 * <p>
 ...
 *     local date time validation capabilities.<br>
 ...
 *     millisecond, nanosecond) for validating local date time values.
 * </p>
 *
 * @author Luis-St
 *
 ...
 */
@FunctionalInterface
public interface LocalDateTimeConstraint<C> extends ApplicableConstraint<LocalDateTimeConstraintConfig, C>, TemporalComparableConstraint<LocalDateTime, C>, TemporalSpanConstraint<LocalDateTime, C>, DateFieldConstraint<LocalDateTime, C>, TimeFieldConstraint<LocalDateTime, C> {
	
	@Override
	@NonNull C apply(@NonNull UnaryOperator<LocalDateTimeConstraintConfig> configModifier);
	
	@Override
	default @NonNull C equalTo(@NonNull LocalDateTime value) {
		return this.apply(config -> config.withEqualTo(value));
	}
	
	@Override
	default @NonNull C notEqualTo(@NonNull LocalDateTime value) {
		return this.apply(config -> config.withNotEqualTo(value));
	}
	
	@Override
	default @NonNull C in(@NonNull Collection<LocalDateTime> values) {
		return this.apply(config -> config.withIn(values));
	}
	
	@Override
	default @NonNull C notIn(@NonNull Collection<LocalDateTime> values) {
		return this.apply(config -> config.withNotIn(values));
	}
	
	@Override
	default @NonNull C custom(@NonNull Constraint<LocalDateTime> constraint) {
		return this.apply(config -> config.withCustom(constraint));
	}
	
	@Override
	default @NonNull C after(@NonNull LocalDateTime value) {
		return this.apply(config -> config.withAfter(value));
	}
	
	@Override
	default @NonNull C afterOrEqual(@NonNull LocalDateTime value) {
		return this.apply(config -> config.withAfterOrEqual(value));
	}
	
	@Override
	default @NonNull C before(@NonNull LocalDateTime value) {
		return this.apply(config -> config.withBefore(value));
	}
	
	@Override
	default @NonNull C beforeOrEqual(@NonNull LocalDateTime value) {
		return this.apply(config -> config.withBeforeOrEqual(value));
	}
	
	@Override
	default @NonNull C between(@NonNull LocalDateTime min, @NonNull LocalDateTime max) {
		return this.apply(config -> config.withBetween(min, max));
	}
	
	@Override
	default @NonNull C betweenOrEqual(@NonNull LocalDateTime min, @NonNull LocalDateTime max) {
