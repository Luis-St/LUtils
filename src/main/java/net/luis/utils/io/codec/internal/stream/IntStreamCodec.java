 ...
 */
@ApiStatus.Internal
public class IntStreamCodec implements Codec<IntStream> {
	...
	@Override
	public @NotNull <R> Result<IntStream> decodeStart(@NotNull TypeProvider<R> provider, @Nullable R value) {
		Objects.requireNonNull(provider, "Type provider must not be null");
		if (value == null) {
			return Result.error("Unable to decode null value as int stream using '" + this + "'");
		}
		
		Result<Stream<Integer>> result = this.internalCodec.decodeStart(provider, value);
		if (result.isError()) {
