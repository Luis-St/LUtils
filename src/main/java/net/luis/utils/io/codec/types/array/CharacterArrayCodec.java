...
public class CharacterArrayCodec extends AbstractCodec<char[], Object> {
	...
	@Override
	public <R> @NotNull Result<R> encodeStart(@NotNull TypeProvider<R> provider, @NotNull R current, char @Nullable [] value) {
	...
	@Override
	public <R> @NotNull Result<char[]> decodeStart(@NotNull TypeProvider<R> provider, @NotNull R current, @Nullable R value) {
		Objects.requireNonNull(provider, "Type provider must not be null");
		Objects.requireNonNull(current, "Current value must not be null");
		if (value == null) {
