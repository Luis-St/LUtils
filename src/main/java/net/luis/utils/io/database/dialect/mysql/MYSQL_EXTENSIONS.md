...
#### setRowFormat
```java
void setRowFormat(@NonNull MysqlRowFormat format);
```
Generates SQL: `ALTER TABLE ... ROW_FORMAT=format`

Sets the row storage format for this table.

#### setKeyBlockSize
```java
void setKeyBlockSize(int sizeKb);
```
Generates SQL: `ALTER TABLE ... KEY_BLOCK_SIZE=sizeKb`

Sets the key block size in kilobytes for compressed tables.

#### setCompression
```java
void setCompression(@NonNull MysqlCompression compression);
```
Generates SQL: `ALTER TABLE ... COMPRESSION='compression'`

Sets the page-level compression algorithm (requires InnoDB and file-per-table tablespace).

#### setEncryption
```java
void setEncryption(boolean enabled);
```
Generates SQL: `ALTER TABLE ... ENCRYPTION='Y'` or `ENCRYPTION='N'`

Enables or disables InnoDB tablespace encryption.

#### setStatsAutoRecalc
```java
void setStatsAutoRecalc(boolean enabled);
```
Generates SQL: `ALTER TABLE ... STATS_AUTO_RECALC=enabled`

Controls whether InnoDB automatically recalculates statistics.

#### setStatsPersistent
```java
void setStatsPersistent(boolean enabled);
```
Generates SQL: `ALTER TABLE ... STATS_PERSISTENT=enabled`

Controls whether InnoDB persists table statistics to disk.
...
void optimizeTable();
```
Generates SQL: `OPTIMIZE TABLE table`

Reorganizes the physical storage of table data and associated indexes to reduce storage and improve I/O efficiency.

#### repairTable
```java
void repairTable();
```
Generates SQL: `REPAIR TABLE table`

Repairs a corrupted table. Only works with MyISAM, ARCHIVE, and CSV tables.

#### checkTable
```java
void checkTable();
```
Generates SQL: `CHECK TABLE table`
...
long checksumTable();
```
Generates SQL: `CHECKSUM TABLE table`

Returns a live checksum of the table contents.
...
```
Generates SQL: `RENAME TABLE table TO newName`

Renames this table atomically.

#### exchange
```java
void exchangePartition(@NonNull String partitionName, @NonNull SqlTable<?> targetTable);
```
Generates SQL: `ALTER TABLE table EXCHANGE PARTITION partitionName WITH TABLE targetTable`

Exchanges a partition with a non-partitioned table. Useful for fast bulk loading.

#### disableKeys
```java
void disableKeys();
```
Generates SQL: `ALTER TABLE table DISABLE KEYS`

Disables non-unique index updates. Speeds up bulk inserts (MyISAM only).

#### enableKeys
```java
void enableKeys();
```
Generates SQL: `ALTER TABLE table ENABLE KEYS`

Re-enables non-unique index updates and rebuilds indexes.
