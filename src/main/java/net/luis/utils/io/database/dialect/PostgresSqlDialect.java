...
/**
 * SQL dialect implementation for PostgreSQL.<br>
 * Provides PostgreSQL-specific SQL generation by extending {@link AbstractSqlDialect}.<br>
 *
 * @author Luis-St
 */
...
public class PostgresSqlDialect extends AbstractSqlDialect {
	...
	/**
	 * The registry of PostgreSQL-specific SQL type mappings.
	 */
...
/**
 * PostgreSQL-specific renderer for comparison conditions.<br>
 * Specializes the rendering of {@code IN} list conditions by binding the options as a PostgreSQL array
 * and using the {@code = ANY(...)} construct when all options share a common element type.<br>
 *
 * @author Luis-St
 */
class PostgresSqlComparisonConditionRenderer extends SqlComparisonConditionRenderer {
	...
	/**
	 * Constructs a new PostgreSQL comparison condition renderer for the given dialect.<br>
	 *
	...
	/**
	 * Determines the common array type for the given list of options.<br>
	 * Returns {@code null} if the options are not all value expressions sharing a single element type that can be represented as an array.<br>
	 *
	 * @param options The expressions to determine the common array type for
	 * @return The common array type or {@code null} if the options have no common array-capable element type
	 */
	...
	/**
	 * Binds the values of the given options as a single array parameter of the given array type to the given renderer.<br>
	 *
	 * @param renderer The renderer to bind the array parameter to
	 * @param arrayType The array type to bind the values as
	 * @param options The expressions whose values are bound as the array elements
...
/**
 * PostgreSQL-specific renderer for table-related SQL.<br>
 * Specializes auto-increment columns using the {@code GENERATED BY DEFAULT AS IDENTITY} clause
 * and renders table truncation with the {@code CASCADE} option.<br>
 *
 * @author Luis-St
 */
class PostgresSqlTableRenderer extends SqlTableRenderer {
	...
	/**
	 * Constructs a new PostgreSQL table renderer for the given dialect.<br>
	 *
	 * @param dialect The dialect this renderer belongs to
	 * @throws NullPointerException If the dialect is null
	 */
...
/**
 * PostgreSQL-specific renderer for temporal functions.<br>
 * Specializes the rendering of the {@code EXTRACT}, from-epoch, make-date and make-time functions
 * using PostgreSQL field names and constructs such as {@code ISODOW}, {@code DOY}, {@code TO_TIMESTAMP},
 * {@code MAKE_DATE} and {@code MAKE_TIME}.<br>
 *
 * @author Luis-St
 */
class PostgresSqlTemporalFunctionRenderer extends SqlTemporalFunctionRenderer {
	...
	/**
	 * Constructs a new PostgreSQL temporal function renderer for the given dialect.<br>
	 *
	 * @param dialect The dialect this renderer belongs to
	 * @throws NullPointerException If the dialect is null
	 */
...
/**
 * PostgreSQL-specific renderer for string conditions.<br>
 * Specializes case-insensitive equality by using the PostgreSQL {@code ILIKE} operator.<br>
 *
 * @author Luis-St
 */
class PostgresSqlStringConditionRenderer extends SqlStringConditionRenderer {
	...
	/**
	 * Constructs a new PostgreSQL string condition renderer for the given dialect.<br>
	 *
	 * @param dialect The dialect this renderer belongs to
	 * @throws NullPointerException If the dialect is null
	 */
...
/**
 * PostgreSQL-specific renderer for numeric functions.<br>
 * Specializes the rendering of the truncate, round, bitwise-xor and logarithm functions using
 * PostgreSQL constructs such as {@code TRUNC}, {@code ROUND} with numeric casts, the {@code #}
 * bitwise operator and {@code LOG}/{@code LN}.<br>
 *
 * @author Luis-St
 */
class PostgresSqlNumericFunctionRenderer extends SqlNumericFunctionRenderer {
	...
	/**
	 * Constructs a new PostgreSQL numeric function renderer for the given dialect.<br>
	 *
	 * @param dialect The dialect this renderer belongs to
	 * @throws NullPointerException If the dialect is null
	 */
...
/**
 * PostgreSQL-specific renderer for string functions.<br>
 * Specializes the rendering of the hex and unhex functions using the PostgreSQL {@code encode}/{@code convert_to}
 * and {@code decode} functions.<br>
 *
 * @author Luis-St
 */
class PostgresSqlStringFunctionRenderer extends SqlStringFunctionRenderer {
	...
	/**
	 * Constructs a new PostgreSQL string function renderer for the given dialect.<br>
	 *
	 * @param dialect The dialect this renderer belongs to
	 * @throws NullPointerException If the dialect is null
	 */
