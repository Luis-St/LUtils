...
class KdfTest {
	...
	private static byte[] derived(KdfAlgorithm algorithm, byte[] ikm, byte[] salt, byte[] info, int length) {
		return materialOf(Kdf.derive(algorithm, ikm, salt, info, length));
	}
	
	@Test
	void constructorIsPrivate() throws Exception {
		Constructor<?>[] constructors = Kdf.class.getDeclaredConstructors();
		assertEquals(1, constructors.length);
		assertTrue(Modifier.isPrivate(constructors[0].getModifiers()));
		assertTrue(Modifier.isFinal(Kdf.class.getModifiers()));
		
		Constructor<Kdf> constructor = Kdf.class.getDeclaredConstructor();
	...
	void extractWithNullAlgorithm() {
		NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.extract(null, SALT, IKM));
		assertEquals("Algorithm must not be null", exception.getMessage());
	}
	
	@Test
	void extractWithNullIkm() {
		NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.extract(ALGORITHM, SALT, null));
		assertEquals("Input key material must not be null", exception.getMessage());
	}
	
	@Test
	void extractWithBothNull() {
		NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.extract(null, SALT, null));
		assertEquals("Algorithm must not be null", exception.getMessage());
	}
	
	@Test
	void extractWithEmptyIkm() {
		byte[] prk = assertDoesNotThrow(() -> materialOf(Kdf.extract(ALGORITHM, SALT, new byte[0])));
		assertEquals(ALGORITHM.outputLength(), prk.length);
	}
	
	@Test
	void expandWithNullAlgorithm() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.expand(null, prk, INFO, 32));
			assertEquals("Algorithm must not be null", exception.getMessage());
		}
	}
	...
	void expandWithNullPrk() {
		NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.expand(ALGORITHM, null, INFO, 32));
	...
	void expandWithZeroLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> Kdf.expand(ALGORITHM, prk, INFO, 0));
	...
	void expandWithNegativeLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			assertThrows(IllegalArgumentException.class, () -> Kdf.expand(ALGORITHM, prk, INFO, -1));
		}
	}
	...
	void expandWithLengthAboveMaximum() {
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			try (Secret prk = Kdf.extract(algorithm, SALT, IKM)) {
				...
				IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> Kdf.expand(algorithm, prk, INFO, maximum + 1));
		...
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			for (int length : new int[] { algorithm.outputLength() - 1, 0 }) {
				try (Secret prk = Secret.adopt(new byte[length])) {
					IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, () -> Kdf.expand(algorithm, prk, INFO, 32));
	...
	void expandWithExactlyOneBlockPrk() {
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			try (Secret prk = Secret.random(algorithm.outputLength())) {
				assertDoesNotThrow(() -> materialOf(Kdf.expand(algorithm, prk, INFO, 32)));
			}
		}
	}
	...
	void expandWithLongerThanOneBlockPrk() {
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			try (Secret prk = Secret.random(2 * algorithm.outputLength())) {
				assertEquals(32, materialOf(Kdf.expand(algorithm, prk, INFO, 32)).length);
			}
		}
	}
	...
	void expandWithClosedPrk() {
		Secret prk = Kdf.extract(ALGORITHM, SALT, IKM);
		prk.close();
		
		IllegalStateException exception = assertThrows(IllegalStateException.class, () -> Kdf.expand(ALGORITHM, prk, INFO, 32));
		assertEquals("Secret has already been closed", exception.getMessage());
	}
	
	@Test
	void deriveWithNullAlgorithm() {
		assertThrows(NullPointerException.class, () -> Kdf.derive(null, IKM, SALT, INFO, 32));
	}
	
	@Test
	void deriveWithNullIkm() {
		assertThrows(NullPointerException.class, () -> Kdf.derive(ALGORITHM, null, SALT, INFO, 32));
	}
	
	@Test
	void deriveWithInvalidLength() {
		assertThrows(IllegalArgumentException.class, () -> Kdf.derive(ALGORITHM, IKM, SALT, INFO, 0));
		assertThrows(IllegalArgumentException.class, () -> Kdf.derive(ALGORITHM, IKM, SALT, INFO, 255 * ALGORITHM.outputLength() + 1));
	}
	
	@Test
	void deriveKeyWithNullTarget() {
		NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, null));
		assertEquals("Target must not be null", exception.getMessage());
	}
	
	@Test
	void deriveKeyWithNullAlgorithm() {
		assertThrows(NullPointerException.class, () -> Kdf.deriveKey(null, IKM, SALT, INFO, AeadAlgorithm.AES_256_GCM));
	}
	
	@Test
	void deriveKeyWithNullIkm() {
		assertThrows(NullPointerException.class, () -> Kdf.deriveKey(ALGORITHM, null, SALT, INFO, AeadAlgorithm.AES_256_GCM));
	}
	
	@Test
	void deriveKeyWithNullTargetAndNullIkm() {
		NullPointerException exception = assertThrows(NullPointerException.class, () -> Kdf.deriveKey(ALGORITHM, null, SALT, INFO, null));
		assertEquals("Target must not be null", exception.getMessage());
	}
	...
	void extractWithNullSalt() {
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			byte[] withoutSalt = materialOf(Kdf.extract(algorithm, null, IKM));
			byte[] withZeroBlock = materialOf(Kdf.extract(algorithm, new byte[algorithm.outputLength()], IKM));
	...
	void extractWithEmptySalt() {
		byte[] withoutSalt = materialOf(Kdf.extract(ALGORITHM, null, IKM));
		assertArrayEquals(withoutSalt, materialOf(Kdf.extract(ALGORITHM, new byte[0], IKM)));
	}
	
	@Test
	void extractWithNonEmptySalt() {
		byte[] salted = materialOf(Kdf.extract(ALGORITHM, SALT, IKM));
		assertEquals(ALGORITHM.outputLength(), salted.length);
		assertFalse(Arrays.equals(salted, materialOf(Kdf.extract(ALGORITHM, null, IKM))));
	}
	
	@Test
	void expandWithNullInfo() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			assertArrayEquals(materialOf(Kdf.expand(ALGORITHM, prk, new byte[0], 32)), materialOf(Kdf.expand(ALGORITHM, prk, null, 32)));
		}
	}
	...
	void expandWithEmptyInfo() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			assertArrayEquals(materialOf(Kdf.expand(ALGORITHM, prk, null, 32)), materialOf(Kdf.expand(ALGORITHM, prk, new byte[0], 32)));
		}
	}
	...
	void expandWithNonEmptyInfo() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			assertFalse(Arrays.equals(materialOf(Kdf.expand(ALGORITHM, prk, null, 32)), materialOf(Kdf.expand(ALGORITHM, prk, INFO, 32))));
		}
	}
	...
	void expandWithSingleBlockLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			byte[] single = materialOf(Kdf.expand(ALGORITHM, prk, INFO, ALGORITHM.outputLength()));
			byte[] longer = materialOf(Kdf.expand(ALGORITHM, prk, INFO, 3 * ALGORITHM.outputLength()));
	...
	void expandWithPartialBlockLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			byte[] block = materialOf(Kdf.expand(ALGORITHM, prk, INFO, ALGORITHM.outputLength()));
			for (int length : new int[] { 1, ALGORITHM.outputLength() - 1 }) {
				byte[] partial = materialOf(Kdf.expand(ALGORITHM, prk, INFO, length));
	...
	void expandWithMultiBlockLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			byte[] two = materialOf(Kdf.expand(ALGORITHM, prk, INFO, 2 * ALGORITHM.outputLength()));
			byte[] three = materialOf(Kdf.expand(ALGORITHM, prk, INFO, 3 * ALGORITHM.outputLength() + 1));
	...
	void expandAtMaximumLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			int maximum = 255 * ALGORITHM.outputLength();
			assertEquals(maximum, materialOf(Kdf.expand(ALGORITHM, prk, INFO, maximum)).length);
		}
	}
	...
	void expandAtMinimumLength() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			assertEquals(1, materialOf(Kdf.expand(ALGORITHM, prk, INFO, 1)).length);
		}
	}
	...
	void extractOutputLength() {
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			assertEquals(algorithm.outputLength(), materialOf(Kdf.extract(algorithm, SALT, IKM)).length);
		}
	}
	
	@Test
	void extractIsDeterministic() {
		assertArrayEquals(materialOf(Kdf.extract(ALGORITHM, SALT, IKM)), materialOf(Kdf.extract(ALGORITHM, SALT, IKM)));
	}
	...
	void extractDiffersForDifferentIkm() {
		...
		other[0] ^= 1;
		assertFalse(Arrays.equals(materialOf(Kdf.extract(ALGORITHM, SALT, IKM)), materialOf(Kdf.extract(ALGORITHM, SALT, other))));
	}
	
	@Test
	void extractDiffersForDifferentSalt() {
		assertFalse(Arrays.equals(materialOf(Kdf.extract(ALGORITHM, SALT, IKM)), materialOf(Kdf.extract(ALGORITHM, "other".getBytes(StandardCharsets.UTF_8), IKM))));
	}
	
	@Test
	void expandIsDeterministic() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			assertArrayEquals(materialOf(Kdf.expand(ALGORITHM, prk, INFO, 42)), materialOf(Kdf.expand(ALGORITHM, prk, INFO, 42)));
		}
	}
	...
	void deriveMatchesExtractThenExpand() {
		byte[] manual;
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			manual = materialOf(Kdf.expand(ALGORITHM, prk, INFO, 42));
		}
	...
	void deriveKeyLength() {
		for (AeadAlgorithm target : AeadAlgorithm.values()) {
			assertEquals(target.keyLength(), Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, target).getEncoded().length);
		}
	}
	...
	void deriveKeyAlgorithmName() {
		for (AeadAlgorithm target : AeadAlgorithm.values()) {
			assertEquals(target.keyJcaName(), Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, target).getAlgorithm());
		}
	}
	...
	void deriveKeyMatchesDerive() {
		...
		assertArrayEquals(derived(ALGORITHM, IKM, SALT, INFO, target.keyLength()), Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, target).getEncoded());
	}
	
	@Test
	void deriveKeyIsDeterministic() {
		AeadAlgorithm target = AeadAlgorithm.AES_256_GCM;
		assertArrayEquals(Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, target).getEncoded(), Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, target).getEncoded());
	}
	...
	void hkdfKnownAnswerVectors() {
		...
		try (Secret prk = Kdf.extract(ALGORITHM, hex.parseHex("000102030405060708090a0b0c"), ikm)) {
			assertEquals("077709362c2e32df0ddc3f0dc47bba6390b6c73bb50f9c3122ec844ad7c2b3e5", hex.formatHex(prk.material()));
			assertEquals("3cb25f25faacd57a90434f64d0362f2a2d2d0a90cf1a5a4c5db02d56ecc4c5bf34007208d5b887185865", hex.formatHex(materialOf(Kdf.expand(ALGORITHM, prk, hex.parseHex("f0f1f2f3f4f5f6f7f8f9"), 42))));
		}
		try (Secret prk = Kdf.extract(ALGORITHM, new byte[0], ikm)) {
			assertEquals("19ef24a32c717b167f33a91d6f648bdf96596776afdb6377ac434c1c293ccb04", hex.formatHex(prk.material()));
			assertEquals("8da4e775a563c18f715f802a063c5a31b8a11f5c5ee1879ec3454e5f3c738d2d9d201395faa4b61a96c8", hex.formatHex(materialOf(Kdf.expand(ALGORITHM, prk, new byte[0], 42))));
		}
	}
	...
	void expandOutputsArePrefixConsistent() {
		...
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			byte[] longest = materialOf(Kdf.expand(ALGORITHM, prk, INFO, 100));
			for (int length : lengths) {
				assertArrayEquals(Arrays.copyOf(longest, length), materialOf(Kdf.expand(ALGORITHM, prk, INFO, length)));
			}
		}
	}
	...
	void deriveKeySurvivesTheClose() {
		SecretKey key = Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, AeadAlgorithm.AES_256_GCM);
	...
	void deriveKeyProducesUsableAeadKey() {
		...
		for (AeadAlgorithm target : AeadAlgorithm.values()) {
			SecretKey key = Kdf.deriveKey(ALGORITHM, IKM, SALT, INFO, target);
			byte[] ciphertext = Aead.encrypt(target, key, message);
			assertArrayEquals(message, Aead.decrypt(target, key, ciphertext));
		}
	}
	...
	void extractDoesNotMutateInputs() {
		...
		materialOf(Kdf.extract(ALGORITHM, salt, ikm));
	...
	void expandDoesNotMutateInfo() {
		...
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			materialOf(Kdf.expand(ALGORITHM, prk, info, 32));
		}
	...
	void expandDoesNotCloseThePrk() {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			byte[] first = materialOf(Kdf.expand(ALGORITHM, prk, INFO, 32));
			
			assertDoesNotThrow(prk::material);
			byte[] second = materialOf(Kdf.expand(ALGORITHM, prk, "other".getBytes(StandardCharsets.UTF_8), 32));
			assertFalse(Arrays.equals(first, second));
	...
	void extractMatchesTheProviderDirectly() throws Exception {
		for (byte[] salt : new byte[][] { null, new byte[0], SALT }) {
			...
			assertArrayEquals(direct, materialOf(Kdf.extract(ALGORITHM, salt, IKM)));
		}
	}
	...
	void expandMatchesTheProviderDirectly() throws Exception {
		try (Secret prk = Kdf.extract(ALGORITHM, SALT, IKM)) {
			for (int length : new int[] { 1, 32, 100 }) {
				...
				assertArrayEquals(direct, materialOf(Kdf.expand(ALGORITHM, prk, INFO, length)));
			}
		}
	}
	...
	void deriveKeyForTheWideNonceTarget() {
		for (KdfAlgorithm algorithm : KdfAlgorithm.values()) {
			SecretKey key = Kdf.deriveKey(algorithm, IKM, SALT, INFO, AeadAlgorithm.XCHACHA20_POLY1305);
			assertEquals(32, key.getEncoded().length);
