...
/**
 * Test class for {@link AuthenticationException}.<br>
 *
 * @author Luis-St
 */
class AuthenticationExceptionTest {
	...
	void constructWithMessage() {
		AuthenticationException exception = new AuthenticationException("failure");
	...
	void constructWithMessageAndCause() {
		...
		AuthenticationException exception = new AuthenticationException("failure", cause);
		assertEquals("failure", exception.getMessage());
	...
	void constructWithNullMessage() {
		AuthenticationException exception = assertDoesNotThrow(() -> new AuthenticationException((String) null));
		assertNull(exception.getMessage());
		assertNull(exception.getCause());
	}
	...
	void constructWithNullCause() {
		AuthenticationException fromCause = assertDoesNotThrow(() -> new AuthenticationException((Throwable) null));
		assertNull(fromCause.getCause());
		assertNull(fromCause.getMessage());
		
		AuthenticationException fromBoth = assertDoesNotThrow(() -> new AuthenticationException("failure", null));
	...
	void constructWithNullMessageAndNullCause() {
		AuthenticationException exception = assertDoesNotThrow(() -> new AuthenticationException(null, null));
		assertNull(exception.getMessage());
		assertNull(exception.getCause());
	}
	...
	void typeHierarchyConsistency() {
		assertInstanceOf(CryptoException.class, new AuthenticationException());
		assertInstanceOf(RuntimeException.class, new AuthenticationException());
	}
	
	@Test
	void constructWithEmptyMessage() {
		assertEquals("", new AuthenticationException("").getMessage());
	}
	...
	void causeChainPreservation() {
		...
		AuthenticationException exception = new AuthenticationException("outer", middle);
	...
	void throwAndCatchAsCryptoException() {
		...
		assertInstanceOf(AuthenticationException.class, caught);
	}
}
