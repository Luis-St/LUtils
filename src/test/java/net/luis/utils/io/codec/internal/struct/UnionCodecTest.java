...
class UnionCodecTest {
	...
	void constructorVarargs() {
		assertThrows(NullPointerException.class, () -> new UnionCodec<>(null, "a", "b"));
		assertThrows(NullPointerException.class, () -> new UnionCodec<>(STRING, (String[]) null));
		assertThrows(IllegalArgumentException.class, () -> new UnionCodec<>(STRING));
		assertDoesNotThrow(() -> new UnionCodec<>(STRING, "a", "b", "c"));
		assertDoesNotThrow(() -> new UnionCodec<>(INTEGER, 1, 2, 3));
	}
	
	@Test
	void encodeStartNullChecks() {
		JsonTypeProvider typeProvider = JsonTypeProvider.INSTANCE;
		Codec<String> codec = new UnionCodec<>(STRING, "pending", "active", "completed");
	...
	void encodeStartWithDifferentTypes() {
		...
		Codec<String> stringCodec = new UnionCodec<>(STRING, "apple", "banana", "cherry");
		...
		Codec<Integer> integerCodec = new UnionCodec<>(INTEGER, 1, 2, 3);
		...
		Codec<Boolean> booleanCodec = new UnionCodec<>(BOOLEAN, true);
	...
	void encodeStartWithSingleValidValue() {
		JsonTypeProvider typeProvider = JsonTypeProvider.INSTANCE;
		Codec<String> codec = new UnionCodec<>(STRING, "only");
	...
	void decodeStartNullChecks() {
		...
		assertThrows(NullPointerException.class, () -> codec.decodeStart(null, new JsonPrimitive("pending")));
	}
	...
	void decodeStartWithValidValue() {
		...
		Result<String> result = codec.decodeStart(typeProvider, new JsonPrimitive("pending"));
		assertTrue(result.isSuccess());
	...
	void decodeStartWithAllValidValues() {
		...
		Result<String> result1 = codec.decodeStart(typeProvider, new JsonPrimitive("pending"));
		assertTrue(result1.isSuccess());
		assertEquals("pending", result1.resultOrThrow());
		
		Result<String> result2 = codec.decodeStart(typeProvider, new JsonPrimitive("active"));
		assertTrue(result2.isSuccess());
		assertEquals("active", result2.resultOrThrow());
		
		Result<String> result3 = codec.decodeStart(typeProvider, new JsonPrimitive("completed"));
		assertTrue(result3.isSuccess());
	...
	void decodeStartWithInvalidValue() {
		...
		Result<String> result = codec.decodeStart(typeProvider, new JsonPrimitive("invalid"));
		assertTrue(result.isError());
		assertTrue(result.errorOrThrow().contains("value is not in the set of valid values"));
		assertTrue(result.errorOrThrow().contains("invalid"));
	}
	...
	void decodeStartWithInvalidType() {
		...
		Result<String> result = codec.decodeStart(typeProvider, new JsonPrimitive(123));
		assertTrue(result.isError());
	}
	...
	void decodeStartWithDifferentTypes() {
		...
		Codec<String> stringCodec = new UnionCodec<>(STRING, "apple", "banana", "cherry");
		Result<String> stringResult = stringCodec.decodeStart(typeProvider, new JsonPrimitive("banana"));
		assertTrue(stringResult.isSuccess());
		assertEquals("banana", stringResult.resultOrThrow());
		
		Codec<Integer> integerCodec = new UnionCodec<>(INTEGER, 1, 2, 3);
		Result<Integer> intResult = integerCodec.decodeStart(typeProvider, new JsonPrimitive(3));
		assertTrue(intResult.isSuccess());
		assertEquals(3, intResult.resultOrThrow());
		
		Codec<Boolean> booleanCodec = new UnionCodec<>(BOOLEAN, true);
		Result<Boolean> boolResult = booleanCodec.decodeStart(typeProvider, new JsonPrimitive(true));
		assertTrue(boolResult.isSuccess());
	...
	void decodeStartWithSingleValidValue() {
		...
		Result<String> validResult = codec.decodeStart(typeProvider, new JsonPrimitive("only"));
		assertTrue(validResult.isSuccess());
		assertEquals("only", validResult.resultOrThrow());
		
		Result<String> invalidResult = codec.decodeStart(typeProvider, new JsonPrimitive("other"));
		assertTrue(invalidResult.isError());
	}
	...
	void decodeStartInnerCodecFailure() {
		JsonTypeProvider typeProvider = JsonTypeProvider.INSTANCE;
		Codec<Integer> codec = new UnionCodec<>(INTEGER, 1, 2, 3);
	...
	void roundTripEncodingDecoding() {
		JsonTypeProvider typeProvider = JsonTypeProvider.INSTANCE;
		Codec<String> codec = new UnionCodec<>(STRING, "red", "green", "blue");
		
		Result<JsonElement> encoded = codec.encodeStart(typeProvider, typeProvider.empty(), "green");
		assertTrue(encoded.isSuccess());
		
		Result<String> decoded = codec.decodeStart(typeProvider, encoded.resultOrThrow());
	...
	void roundTripWithMultipleValues() {
		JsonTypeProvider typeProvider = JsonTypeProvider.INSTANCE;
		Codec<Integer> codec = new UnionCodec<>(INTEGER, 10, 20, 30, 40, 50);
		...
		for (int value : Arrays.asList(10, 20, 30, 40, 50)) {
			...
			Result<Integer> decoded = codec.decodeStart(typeProvider, encoded.resultOrThrow());
			assertTrue(decoded.isSuccess());
	...
	void factoryMethodFromCodecInterface() {
		Codec<String> codec = Codec.union(STRING, "a", "b", "c");
		assertNotNull(codec);
	...
	@Test
	void factoryMethodWithCollection() {
		Set<String> validValues = Set.of("x", "y", "z");
		Codec<String> codec = Codec.union(STRING, validValues);
		assertNotNull(codec);
		assertInstanceOf(UnionCodec.class, codec);
	...
	void equalsAndHashCode() {
		UnionCodec<String> codec1 = new UnionCodec<>(STRING, "a", "b", "c");
		UnionCodec<String> codec2 = new UnionCodec<>(STRING, "a", "b", "c");
		UnionCodec<String> codec3 = new UnionCodec<>(STRING, "a", "b", "c", "d");
	...
	void equalityWithDifferentCollectionTypes() {
		...
		UnionCodec<String> codecFromVarargs = new UnionCodec<>(STRING, "a", "b", "c");
	...
	void toStringRepresentation() {
		UnionCodec<String> codec = new UnionCodec<>(STRING, "pending", "active", "completed");
