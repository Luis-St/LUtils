...
class LocalDateTimeCodecTest {
	...
	void decodeStartNullChecks() {
		...
		assertThrows(NullPointerException.class, () -> codec.decodeStart(null, new JsonPrimitive("2025-01-15T10:30:00")));
	}
	...
	void decodeStartWithNull() {
		...
		Result<LocalDateTime> result = codec.decodeStart(typeProvider, null);
		assertTrue(result.isError());
	...
	void decodeStartWithValidString() {
		...
		Result<LocalDateTime> result = codec.decodeStart(typeProvider, new JsonPrimitive("2025-01-15T10:30:00"));
		assertTrue(result.isSuccess());
	...
	void decodeStartWithEpoch() {
		...
		Result<LocalDateTime> result = codec.decodeStart(typeProvider, new JsonPrimitive("1970-01-01T00:00:00"));
		assertTrue(result.isSuccess());
	...
	void decodeStartWithNanoseconds() {
		...
		Result<LocalDateTime> result = codec.decodeStart(typeProvider, new JsonPrimitive("2025-01-15T10:30:00.123456789"));
		assertTrue(result.isSuccess());
	...
	void decodeStartWithInvalidFormat() {
		...
		Result<LocalDateTime> result = codec.decodeStart(typeProvider, new JsonPrimitive("invalid-datetime"));
		assertTrue(result.isError());
	...
	void decodeStartWithNonString() {
		...
		Result<LocalDateTime> result = codec.decodeStart(typeProvider, new JsonPrimitive(42));
		assertTrue(result.isError());
	}
