...
import static net.luis.utils.io.codec.Codecs.BYTE;
import static org.junit.jupiter.api.Assertions.*;

/**
 * Test class for {@link ByteCodec} focusing on integer constraint functionality.<br>
 *
 * @author Luis-St
 */
class ConstrainedByteCodecTest {
	...
	void encodeWithPositiveConstraintSuccess() {
		Codec<Byte> codec = BYTE.positive();
		byte value = 10;
	...
	void encodeWithNegativeConstraintSuccess() {
		Codec<Byte> codec = BYTE.negative();
		byte value = -10;
	...
	void encodeWithNonNegativeConstraintSuccess() {
		Codec<Byte> codec = BYTE.nonNegative();
		byte value = 0;
	...
	void encodeWithDivisibleByConstraintSuccess() {
		Codec<Byte> codec = BYTE.divisibleBy(5);
		byte value = 25;
	...
	void encodeWithPowerOfTwoConstraintSuccess() {
		Codec<Byte> codec = BYTE.powerOfTwo();
		byte value = 64;
	...
	void decodeWithPositiveConstraintSuccess() {
		...
		Result<Byte> result = codec.decodeStart(this.provider, this.provider.empty(), json);
		assertTrue(result.isSuccess());
		assertEquals((byte) 10, result.resultOrThrow());
	}
	...
	void decodeWithDivisibleByConstraintSuccess() {
		...
		Result<Byte> result = codec.decodeStart(this.provider, this.provider.empty(), json);
		assertTrue(result.isSuccess());
		assertEquals((byte) 25, result.resultOrThrow());
	}
	...
	void encodeWithChainedConstraintsSuccess() {
		Codec<Byte> codec = BYTE.positive().even().divisibleBy(10);
		byte value = 50;
	...
	void decodeWithChainedConstraintsSuccess() {
		Codec<Byte> codec = BYTE.nonNegative().even();
		JsonPrimitive json = new JsonPrimitive(10);
	...
	void decodeWithChainedConstraintsFailure() {
		Codec<Byte> codec = BYTE.positive().even();
		JsonPrimitive json = new JsonPrimitive(7);
	...
	void toStringWithConstraints() {
		Codec<Byte> codec = BYTE.positive().even();
		String str = codec.toString();
		assertTrue(str.startsWith("ConstrainedByteCodec["));
