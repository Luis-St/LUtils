...
/**
 * Test class for {@link DisconnectEventHandler}.<br>
 *
 * @author Luis-St
 */
class DisconnectEventHandlerTest {
	...
	void handleReceivesConnectionEndpointsAndTimestamp() {
		...
		DisconnectEventHandler handler = (conn, local, remote, time) -> {
	...
	void handleAcceptsNullConnectionWhenNotAvailable() {
		...
		DisconnectEventHandler handler = (conn, local, remote, time) -> capturedConnection.set(conn);
	...
	void handleAcceptsHostEndpoints() {
		...
		DisconnectEventHandler handler = (conn, first, second, time) -> {
	...
	void handlerExceptionPropagatesToCaller() {
		...
		RuntimeException exception = assertThrows(RuntimeException.class, () -> handler.handle(null, LOCAL, REMOTE, Instant.now()));
		assertEquals("handler failed", exception.getMessage());
	}
	
	@Test
	void handlerUsableAsLambdaAndMethodReference() {
		DisconnectEventHandler lambda = (conn, local, remote, time) -> {};
		DisconnectEventHandler methodReference = DisconnectEventHandlerTest::ignoreEvent;
