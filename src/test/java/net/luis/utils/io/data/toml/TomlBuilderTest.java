...
class TomlBuilderTest {
	...
	void tableFactory() {
		...
		assertTrue(builder.isInTableContext());
	...
	@Test
	void arrayFactory() {
		TomlBuilder builder = TomlBuilder.array();
		assertNotNull(builder);
		assertTrue(builder.isInArrayContext());
		assertFalse(builder.isInTableContext());
		assertTrue(builder.isAtRootLevel());
		assertEquals(0, builder.getNestingDepth());
	}
	...
	void createEmptyTable() {
		TomlTable table = TomlBuilder.table().buildTable();
	...
	void addAllPrimitiveTypes() {
		TomlTable table = TomlBuilder.table()
			...
			.add("nullNumberKey", (Number) null).buildTable();
	...
	void addDateTimeTypes() {
		...
		TomlTable table = TomlBuilder.table()
			...
			.add("nullOffsetKey", (OffsetDateTime) null).buildTable();
	...
	void addTomlElement() {
		TomlTable table = TomlBuilder.table()
			...
			.add("tableKey", new TomlTable()).buildTable();
	...
	void conditionalBuilding() {
		TomlTable table = TomlBuilder.table()
			...
			.addIf(false, "numExcluded", 0).buildTable();
	...
	void existingNestedTableExtension() {
		TomlTable table = TomlBuilder.table().startTable("nested").add("key1", "value1").endTable().startTable("nested").add("key2", "value2").endTable().buildTable();
	...
	@Test
	void endTableInArrayContextThrows() {
		TomlBuilder builder = TomlBuilder.array();
		assertThrows(IllegalStateException.class, builder::endTable);
	}
	
	@Test
	void endArrayInTableContextThrows() {
		TomlBuilder builder = TomlBuilder.table();
		assertThrows(IllegalStateException.class, builder::endArray);
	}
	...
	void addArrayOfTablesVarargs() {
		...
		TomlTable root = TomlBuilder.table()
			.addArrayOfTables("servers", table1, table2).buildTable();
	...
	void addArrayOfTablesList() {
		...
		TomlTable root = TomlBuilder.table()
			.addArrayOfTables("items", tables).buildTable();
	...
	@Test
	void startArrayOfTables() {
		TomlTable root = TomlBuilder.table().startArrayOfTables("servers").startTable().add("name", "server1")
			.endTable().startTable().add("name", "server2")
			.endTable().endArrayOfTables().buildTable();
		
		TomlArray servers = root.getTomlArray("servers");
		assertTrue(servers.isArrayOfTables());
		assertEquals(2, servers.size());
		assertEquals("server1", servers.getAsTomlTable(0).getAsString("name"));
		assertEquals("server2", servers.getAsTomlTable(1).getAsString("name"));
	}
	
	@Test
	void createEmptyArray() {
		TomlArray array = TomlBuilder.array().buildArray();
	...
	@Test
	void arrayAddAllStrings() {
		TomlArray array = TomlBuilder.array().addAll("a", "b", "c").buildArray();
		
		assertEquals(3, array.size());
		assertEquals("a", array.getAsString(0));
		assertEquals("b", array.getAsString(1));
		assertEquals("c", array.getAsString(2));
	}
	
	@Test
	void arrayAddAllNumbers() {
		TomlArray array = TomlBuilder.array().addAll(1, 2, 3).buildArray();
	...
	@Test
	void arrayAddAllBooleans() {
		TomlArray array = TomlBuilder.array().addAll(true, false, true).buildArray();
		
		assertEquals(3, array.size());
		assertTrue(array.getAsBoolean(0));
		assertFalse(array.getAsBoolean(1));
		assertTrue(array.getAsBoolean(2));
	}
	...
	void arrayAddAllNullValidation() {
		TomlBuilder builder = TomlBuilder.array();
		assertThrows(NullPointerException.class, () -> builder.addAll((TomlArray) null));
	}
	...
	void arrayConditionalAdding() {
		TomlArray array = TomlBuilder.array()
			.addIf(true, "included")
			.addIf(false, "excluded").addIf(true, true).addIf(false, false).addIf(true, 42).addIf(false, 99).buildArray();
	...
	@Test
	void startTableInArray() {
		TomlArray result = TomlBuilder.array().add("first").startTable().add("id", 1).add("name", "Item").endTable().add("last").buildArray();
		
		assertEquals(3, result.size());
		assertEquals("first", result.getAsString(0));
		assertTrue(result.get(1).isTomlTable());
		assertEquals("last", result.getAsString(2));
	}
	
	@Test
	void startArrayInTable() {
		TomlTable result = TomlBuilder.table().add("name", "John").startArray("hobbies").add("reading").add("coding").endArray().buildTable();
		
		assertEquals(2, result.size());
		assertTrue(result.get("hobbies").isTomlArray());
		assertEquals(2, result.getTomlArray("hobbies").size());
	}
	
	@Test
	void startArrayInArray() {
		TomlArray result = TomlBuilder.array().add("outer").startArray().add("inner1").add("inner2").endArray().buildArray();
		
		assertEquals(2, result.size());
		assertTrue(result.get(1).isTomlArray());
		assertEquals(2, result.get(1).getAsTomlArray().size());
	}
	
	@Test
	void deeplyNestedStructure() {
		TomlTable result = TomlBuilder.table().add("level", 1).startTable("nested").add("level", 2).startArray("items").startTable().add("level", 3).endTable().endArray().endTable().buildTable();
		
		assertEquals(2, result.size());
		TomlTable nested = result.getTomlTable("nested");
		assertEquals(new TomlValue(2), nested.get("level"));
		TomlArray items = nested.getTomlArray("items");
		assertEquals(1, items.size());
		assertTrue(items.get(0).isTomlTable());
	}
	
	@Test
	void contextValidationInTableMode() {
		TomlBuilder builder = TomlBuilder.table();
		
		assertDoesNotThrow(() -> builder.add("key", "value"));
		assertDoesNotThrow(() -> builder.startTable("nested"));
		assertDoesNotThrow(() -> builder.endTable());
		
		assertThrows(IllegalStateException.class, () -> builder.add("value"));
		assertThrows(IllegalStateException.class, () -> builder.add(42));
		assertThrows(IllegalStateException.class, () -> builder.startTable());
	...
	void contextValidationInArrayMode() {
		...
		assertDoesNotThrow(() -> builder.startTable());
		assertDoesNotThrow(() -> builder.endTable());
		
		assertThrows(IllegalStateException.class, () -> builder.add("key", "value"));
		assertThrows(IllegalStateException.class, () -> builder.startTable("key"));
	...
	void buildThrowsWithUnclosedContexts() {
		TomlBuilder builder = TomlBuilder.table().startTable("nested").add("inner", "data");
		
		assertThrows(IllegalStateException.class, builder::build);
	}
	
	@Test
	void buildTableFromArrayThrows() {
		TomlBuilder builder = TomlBuilder.array().add("item");
		assertThrows(IllegalStateException.class, builder::buildTable);
	}
	
	@Test
	void buildArrayFromTableThrows() {
	...
	void buildMultipleTimesReturnsConsistentResults() {
		...
		TomlElement first = builder.build();
		TomlElement second = builder.build();
		TomlTable firstTable = builder.buildTable();
		TomlTable secondTable = builder.buildTable();
	...
	void arrayMethodChainingConsistency() {
		...
		assertSame(builder, builder.addIf(true, true));
		assertSame(builder, builder.addIf(true, 42));
