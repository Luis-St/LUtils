...
/**
 * Test class for {@link PostgresSqlTableRenderer}.<br>
 *
 * @author Luis-St
 */
class PostgresSqlTableRendererTest {
	
	private static final PostgresSqlTableRenderer RENDERER = new PostgresSqlTableRenderer(SqlDialects.POSTGRESQL);
	...
	@Test
	void renderAutoIncrementProducesGeneratedIdentity() throws SqlException {
		SqlRenderer renderer = SqlRenderer.empty();
		RENDERER.renderAutoIncrement(renderer, SqlTestFixtures.integerColumn());
		String sql = renderer.toSql().sql();
		assertTrue(sql.contains("GENERATED"));
		assertTrue(sql.contains("BY"));
