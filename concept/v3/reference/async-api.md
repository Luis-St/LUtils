...
SqlDatabaseConfig config = SqlDatabaseConfig.builder()
...
CompletableFuture<User> user = UserTable.TABLE.select()
    ...
    .fetchOneAsync()
    .exceptionally(e -> {
        if (e.getCause() instanceof SqlEntityNotFoundException) {
...
```java
public interface SqlSelectBuilder<E> {
...
public interface SqlUpdateBuilder<E> {
    int execute();
    CompletableFuture<Integer> executeAsync();
}

public interface SqlDeleteBuilder<E> {
    int execute();
    CompletableFuture<Integer> executeAsync();
}
```
