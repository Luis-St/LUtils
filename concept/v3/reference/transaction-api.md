...
public interface SqlTransaction extends AutoCloseable {
     ...
     */
    void rollbackTo(SqlSavepoint savepoint);
     ...
     */
    void release(SqlSavepoint savepoint);
     ...
     */
    SqlTransaction isolation(SqlIsolationLevel level);
     ...
     * @return This transaction for chaining
     */
    SqlTransaction timeout(Duration duration);
...
public interface SqlDatabase {
     ...
     * @return A new transaction
     */
    SqlTransaction beginTransaction(SqlIsolationLevel level);
     ...
     */
    <T> T inTransaction(Function<SqlTransaction, T> action);
     ...
     * @param action The action to execute
     */
    void inTransaction(Consumer<SqlTransaction> action);
}
```
...
void innerMethod() {
    ...
    if (SqlTransaction.isInTransaction()) {
        // Get the current transaction
        SqlTransaction current = SqlTransaction.current()
            .orElseThrow(() -> new IllegalStateException("Expected active transaction"));

        // Or require it explicitly
        SqlTransaction.requireActive();  // Throws if no active transaction
...
public interface SqlTransaction {
     ...
     */
    static Optional<SqlTransaction> current();
     ...
     */
    static SqlTransaction requireActive();
...
try (SqlTransaction tx = database.beginTransaction().readOnly()) {
    List<User> users = UserTable.TABLE.select()
        .where(UserTable.STATUS.equalTo(UserStatus.ACTIVE))
        .fetch();

    // Any insert/update/delete would throw SqlReadOnlyTransactionException
}
```
...
```java
try (SqlTransaction tx = database.beginTransaction().timeout(Duration.ofSeconds(30))) {
    // Long-running operations
    processLargeDataset();
    tx.commit();
}
// SqlTransactionTimeoutException is thrown if timeout is exceeded
```
...
try (SqlTransaction tx = database.beginTransaction()) {
    ...
    // General SqlDatabaseException is also rolled back by try-with-resources
}
```
...
```java
database.addTransactionListener(new SqlTransactionListener() {
    @Override
    public void beforeCommit(SqlTransaction tx) {
        // Called before commit
        log.info("About to commit transaction");
    }

    @Override
    public void afterCommit(SqlTransaction tx) {
        // Called after successful commit
        cacheInvalidator.invalidate(tx.getChangedEntities());
    }

    @Override
    public void afterRollback(SqlTransaction tx) {
...
CompletableFuture<User> result = database.inTransactionAsync(tx -> {
    return UserTable.TABLE.insertAsync(user)
        .thenCompose(saved -> {
            SqlAuditEntry entry = SqlAuditEntry.forCreate(saved);
...
User user = SqlRetry.withBackoff(3, Duration.ofMillis(100))
    .retryOn(SqlDeadlockException.class)
    .execute(() -> {
        try (SqlTransaction tx = database.beginTransaction()) {
